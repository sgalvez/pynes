...
def test_cli_version_uses_generated_build_metadata(
    ...
    monkeypatch.setattr("nes_py.cli.get_version", lambda: "1.0.2.dev42+gabc1234")

    with pytest.raises(SystemExit) as exc_info:
        main(["--version"])

    assert exc_info.value.code == 0
    assert "nes-py 1.0.2.dev42+gabc1234" in capsys.readouterr().out


def test_build_info_reads_generated_metadata(monkeypatch: pytest.MonkeyPatch) -> None:
    metadata = SimpleNamespace(
        version="1.0.2.dev42+gabc1234",
        base_version="1.0.2",
    ...
    assert get_build_info()["version"] == "1.0.2.dev42+gabc1234"
    assert get_version() == "1.0.2.dev42+gabc1234"
